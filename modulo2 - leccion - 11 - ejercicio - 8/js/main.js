...
function getDataLS() {
    ...
    if (characterLS !== null) {
        // METER AQUÍ Bucle clásico
        if (characterLS.name.includes(userInput)) {
            console.log(
                "Encontrada en localStorage info con ese mismo nombre. Se muestra en pantalla."
            );
            list.innerHTML += `<li>Nombre del personaje: ${characterLS.name}`;
            list.innerHTML += `Género del personaje: ${characterLS.gender}</li>`;
        } else {
            console.log(
